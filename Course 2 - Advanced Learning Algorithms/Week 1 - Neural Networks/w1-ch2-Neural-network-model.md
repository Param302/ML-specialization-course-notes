...
| $a^{[l]}_j = g(\vec{w}_j^{[l]} \cdot \vec{a}^{[l-1]} + b_j^{[l]})$ | $g$ is the activation function <br/> $a^{[l]}$ is the activation vector of the previous ($l-1^{th}$) layer <br/> $\vec{a}^{[l-1]}_j$ is the activation output of previous $l-1$ layer |
